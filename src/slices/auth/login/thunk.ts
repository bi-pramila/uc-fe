...
>(
  ...
  async (credentials, thunkAPI) => {
    try {
      ...
      const response = await loginUserService(credentials.email, credentials.password, credentials.captcha);
...
export const logoutUser = createAsyncThunk<import("./types").LogoutResponse, void, { rejectValue: string }>(
  "auth/logoutUser",
  async (_, thunkAPI) => {
    try {
