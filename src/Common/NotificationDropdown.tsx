    ...
    return (
        <>
            <Dropdown className="relative flex items-center h-header">
                ...
                <Dropdown.Content placement="right-end" className="absolute z-50 ltr:text-left rtl:text-right bg-white rounded-md shadow-md !top-4 dropdown-menu min-w-[20rem] lg:min-w-[26rem] dark:bg-zinc-600" aria-labelledby="notificationDropdown">
                    <div className="p-4">
                        ...
                        <div className="flex flex-col gap-1">
                            {
                                (notification || []).filter((data: Notification) => filter === "all" || data.type === filter)?.map((item: Notification, index: number) => (
                                    <a key={index} href="#!" className="flex gap-3 p-4 product-item hover:bg-slate-50 dark:hover:bg-zinc-500">
                                        ...
                                        <div className="grow">
                                            ...
                                            <p className={`text-sm text-slate-500 dark:text-zinc-300 ${index === notification.length - 3 ? "mb-3" : "mb-0"}`}>
                                                <Clock className="inline-block size-3 mr-1"></Clock> <span className="align-middle">{item.date}</span></p>
                                            {item.description && <div className="p-2 rounded bg-slate-100 text-slate-500 dark:bg-zinc-500 dark:text-zinc-300">{item.description}</div>}
                                        </div>
                                        <div className="flex items-center self-start gap-2 text-xs text-slate-500 shrink-0 dark:text-zinc-300">
                                            <div className="w-1.5 h-1.5 bg-fecustom-500 rounded-full"></div> {item.time}
                                        </div>
                                    </a>
                                ))
                            }
