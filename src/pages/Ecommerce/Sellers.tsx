            ...
            <form action="#!" className="mb-5">
                <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
                    ...
                    <div className="ltr:lg:text-right rtl:lg:text-left lg:col-span-3 lg:col-start-10">
                        <button data-modal-target="addSellerModal" type="button" className="text-white btn bg-fecustom-500 border-fecustom-500 hover:text-white hover:bg-fecustom-600 hover:border-fecustom-600 focus:text-white focus:bg-fecustom-600 focus:border-fecustom-600 focus:ring focus:ring-fecustom-100 active:text-white active:bg-fecustom-600 active:border-fecustom-600 active:ring active:ring-fecustom-100 dark:ring-fecustom-400/20" onClick={toggle}>
                            ...
                            <div className="flex items-center gap-2 mb-4">
                                <div className="grow">
                                    <Link to="#!" className={`group/item toggle-button ${item.isFav && "active"}`} onClick={(e) => btnFav(e.target)}>
                                        <Heart className="size-5 text-slate-500 dark:text-zinc-200 fill-slate-200 dark:fill-zinc-500 transition-all duration-150 ease-linear group-[.active]/item:text-yellow-500 dark:group-[.active]/item:text-yellow-500 group-[.active]/item:fill-yellow-200 dark:group-[.active]/item:fill-yellow-500/20 group-hover/item:text-yellow-500 dark:group-hover/item:text-yellow-500 group-hover/item:fill-yellow-200 dark:group-hover/item:fill-yellow-500/20" />
                                    </Link>
                                </div>
                                <Dropdown className="relative dropdown shrink-0">
                                    ...
                                    <Dropdown.Content placement="right-end" className="absolute z-50 py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zinc-600" aria-labelledby="sellersAction1">
                                        <li>
                                            <Link className="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zinc-100 dark:hover:bg-zinc-500 dark:hover:text-zinc-200 dark:focus:bg-zinc-500 dark:focus:text-zinc-200" to="#!"><Eye className="inline-block size-3 mr-1" /> <span className="align-middle">Overview</span></Link>
                                        </li>
                                        <li>
                                        ...
                                        <li>
                                            <Link className="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zinc-100 dark:hover:bg-zinc-500 dark:hover:text-zinc-200 dark:focus:bg-zinc-500 dark:focus:text-zinc-200" to="#!" onClick={() => onClickDelete(item)}><Trash2 className="inline-block size-3 mr-1" /> <span className="align-middle">Delete</span></Link>
                                        </li>
                                    </Dropdown.Content>
                                </Dropdown>
                            </div>
                            <div className="flex items-center justify-center size-16 mx-auto rounded-full bg-slate-100 outline outline-slate-100 outline-offset-1 dark:bg-zinc-600 dark:outline-zinc-600">
                                <img src={item.logo} alt="" className="h-10 rounded-full" />
                            </div>

                            <div className="mt-4 mb-6 text-center">
                                <h6 className="text-16"><Link to="#!">{item.companyName}</Link></h6>
                                <p className="text-slate-500 dark:text-zinc-200">{item.name}</p>
                            </div>
                            <div className="grid grid-cols-1 gap-5 text-center divide-y md:divide-y-0 md:divide-x sm:grid-cols-3 2xl:grid-cols-12 divide-slate-200 divide-dashed dark:divide-zinc-500 rtl:divide-x-reverse">
                                <div className="p-2 2xl:col-span-4">
                                    <h6 className="mb-1">{item.sales}</h6>
                                    <p className="text-slate-500 dark:text-zinc-200">Sales</p>
                                </div>
                                <div className="p-2 2xl:col-span-4">
                                    <h6 className="mb-1">{item.products}</h6>
                                    <p className="text-slate-500 dark:text-zinc-200">Product</p>
                                </div>
                                <div className="p-2 2xl:col-span-4">
                                    <h6 className="mb-1">{item.revenue}</h6>
                                    <p className="text-slate-500 dark:text-zinc-200">Revenue</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <Modal.Body className="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                    <form action="#!" onSubmit={(e) => {
                        ...
                        <div className="mb-3">
                            ...
                            >
                                {({ getRootProps }: any) => (
                                    <div className="flex items-center justify-center bg-white border border-dashed rounded-md cursor-pointer dropzone border-slate-200 dropzone2 dark:bg-zinc-600 dark:border-zinc-500">
                                        <div className="w-full py-5 text-lg text-center dz-message needsclick" {...getRootProps()} >
                                            ...
                                            <h5 className="mb-0 font-normal text-slate-500 dark:text-zinc-200 text-15">Drag and drop your logo or <Link to="#!">browse</Link> your logo</h5>
                                        </div>
                                    </div>
                                )}
                            </Dropzone>
                                            ...
                                            <div className="border rounded border-slate-200 dark:border-zinc-500">
                                                <div className="p-2 text-center">
                                                    ...
                                                    <div className="pt-3">
                                                        ...
                                                        <p className="mb-0 text-slate-500 dark:text-zinc-200" data-dz-size>{selectfiles.formattedSize}</p>
                        ...
                        <div className="mb-3">
                            ...
                            <input type="text" id="ownerName" className="form-input border-slate-200 dark:border-zinc-500 focus:outline-none focus:border-fecustom-500 disabled:bg-slate-100 dark:disabled:bg-zinc-600 disabled:border-slate-300 dark:disabled:border-zinc-500 dark:disabled:text-zinc-200 disabled:text-slate-500 dark:text-zinc-100 dark:bg-zinc-700 dark:focus:border-fecustom-800 placeholder:text-slate-400 dark:placeholder:text-zinc-200" placeholder="Owner name"
                                name="name"
                                onChange={validation.handleChange}
